...
//---------------------------------------------------------------------------
const u8 CRC8_LOOKUP_TABLE[256] ={//查表法(生成多项式x8+x2+x+1)
...
/**
	...
	* @注	
*/
u8 calc_crc8(void *dataPtr,u16 dataLen){//查表法
   u8 carry = 0;
   while(dataLen--){
...
bool IIC_ReadDataWithCRC(u8 regAddr,void *dataPtr,u16 dataLen){
  ...
  if(dataLen<=(sizeof(I2CTransferBuffer)<<1) && IIC_TransferDataRaw(dummyWr,2,I2CTransferBuffer,dataLen<<1))	//先写后读
	{  
