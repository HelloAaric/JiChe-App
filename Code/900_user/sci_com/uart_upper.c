...
void Sci_ACK_0x03_ReadRegs_Data(struct RS485MSG *s,UINT8 t_u8BuffTemp[]) {
	...
	// u16SciTemp = (UINT16)(Switch_OnOFF_Func.all&0x0000FFFF);
	u16SciTemp = 0;
	t_u8BuffTemp[i++] = (u16SciTemp>>8)& 0x00FF;
	t_u8BuffTemp[i++] = u16SciTemp & 0x00FF;

	// u16SciTemp = (UINT16)(Switch_OnOFF_Func.all>>16);
	u16SciTemp = 0;
	t_u8BuffTemp[i++] = (u16SciTemp>>8)& 0x00FF;
	t_u8BuffTemp[i++] = u16SciTemp & 0x00FF;
...
void Sci_ACK_0x03_R_PC_LCD(UINT8 t_u8BuffTemp[])
{
	Uart_R_PC_LCD_ACK_0x03(t_u8BuffTemp);
}
...
void Sci_ACK_0x03_RW_Data_Other(struct RS485MSG *s,UINT8 t_u8BuffTemp[]) {	//86
	...
	UINT16	i,j;
	i = 0;
	for(j = 0; j < SOC_Size_TableCanSet; j++) {		//由于GetEndValue()函数的问题，只能混在一起
...
void Sci_ACK_0x03( struct RS485MSG *s ) {
	...
	if(s->AckType == RS485_ACK_POS) {
		// if(s->u16RdRegStartAddrActure >= RS485_ADDR_RW_CALIB) {
			...
			else if(s->u16RdRegStartAddrActure >= RS485_ADDR_RW_CALIB) {
				Sci_ACK_0x03_RW_Data_Cali(s,g_u8SCITxBuff);
			} else if(s->u16RdRegStartAddrActure >= 128) {
				Sci_ACK_0x03_R_PC_LCD(g_u8SCITxBuff);
			} else {
...
void Sci1_CommonUpper_FaultChk(void)
{
	...
	if(UART_UPPER1->STS&&(1<<13)) {		//接收溢出错误，RXNEIE或EIE使能产生中断，开
		(void)UART_UPPER1->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER1->DAT;
		FaultCnt++;
	}

	if(UART_UPPER1->STS&&(1<<14)) {		//检测到噪声，默认开，不开的话CR3的ONEBIT置1，不开
		(void)UART_UPPER1->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER1->DAT;
		FaultCnt++;
	}

	if(UART_UPPER1->STS&&(1<<15)) {		//帧错误，USART_CR3的EIE使能中断，开
		(void)UART_UPPER1->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER1->DAT;
		FaultCnt++;
	}

	if(UART_UPPER1->STS&&(1<<12)) {		//奇偶校验错误标志 USART_CTRL1的PEIE使能该中断，不开
		(void)UART_UPPER1->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER1->DAT;
		FaultCnt++;
	}
...
void Sci1_CommonUpper_Rx_Deal(struct RS485MSG *s) {
	...
	//NVIC_DisableIRQ(UART_UPPER_IRQn);	
	UART_UPPER1->CTRL1 &= ~(1<<9);		//和上面那句话二选一
	s->u16Buffer[s->ptr_no] = UART_UPPER1->DAT; 				// 从RXFIFO 中读取接收到的数据
			...
			switch(s->enRs485CmdType) {
				...
				case RS485_CMD_WRITE_REG:
					if (s->ptr_no == 7) {	//	receive complete
						...
						//RC1IE = 0;			// 禁止EUSART2 接收中断
						UART_UPPER1->CTRL1 &= ~(1<<1);
						UART_UPPER1->CTRL1 &= ~(1<<9);
					}
					break;
				case RS485_CMD_WRITE_REGS:
...
void Sci1_CommonUpper_Tx_Deal(struct RS485MSG *s) {
	...
	// 这里不行
	// RS485_EN = 1;
	while(!((UART_UPPER1->STS)&(1<<7)));				// 1<<6 也可以
	if( s->ptr_no < s -> AckLenth) {
        UART_UPPER1->DAT = s->u16Buffer[s->ptr_no];	//load data
		s->ptr_no++;
...
void App_CommonUpperSCI1(struct RS485MSG *s) {
	...
	switch(s->csr) {
		...
		case RS485_STA_TX_COMPLETE: {
			if(gu8_TxFinishFlag_SCI1) {
				...
				s->ptr_no = 0;
				UART_UPPER1->CTRL1 |= (1<<1);	//使能接收
				UART_UPPER1->CTRL1 |= (1<<9);	//使能接收中断
				gu8_TxEnable_SCI1 = 0;
				MCUO_RS485_EN = 1;
			}
			break;
	...
	// Sci1_CommonUpper_FaultChk();	//没必要在这加
}
...
void Sci2_CommonUpper_FaultChk(void) {
	...
	if(UART_UPPER2->STS&&(1<<13)) {		//接收溢出错误，RXNEIE或EIE使能产生中断，开
		(void)UART_UPPER2->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER2->DAT;
		FaultCnt++;
	}

	if(UART_UPPER2->STS&&(1<<14)) {		//检测到噪声，默认开，不开的话CR3的ONEBIT置1，不开
		(void)UART_UPPER2->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER2->DAT;
		FaultCnt++;
	}

	if(UART_UPPER2->STS&&(1<<15)) {		//帧错误，USART_CR3的EIE使能中断，开
		(void)UART_UPPER2->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER2->DAT;
		FaultCnt++;
	}

	if(UART_UPPER2->STS&&(1<<12)) {		//奇偶校验错误标志 USART_CTRL1的PEIE使能该中断，不开
		(void)UART_UPPER2->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER2->DAT;
		FaultCnt++;
	}
...
void Sci2_CommonUpper_Rx_Deal(struct RS485MSG *s) {
	...
	UART_UPPER2->CTRL1 &= ~(1<<9);		//和上面那句话二选一
	s->u16Buffer[s->ptr_no] = UART_UPPER2->DAT; 				// 从RXFIFO 中读取接收到的数据
	if((s->ptr_no == 0)&&(s->u16Buffer[0] != RS485_SLAVE_ADDR)&&(s->u16Buffer[0] != RS485_BROADCAST_ADDR)) {
		s->ptr_no = 0;
		s->u16Buffer[0] = 0;
	}
			...
			switch(s->enRs485CmdType) {
				...
				case RS485_CMD_WRITE_REG:
					if (s->ptr_no == 7) {	//	receive complete
						...
						//RCSTA1bits.CREN = 0;  //禁止接收
						//RC1IE = 0;			// 禁止EUSART2 接收中断
						UART_UPPER2->CTRL1 &= ~(1<<1);
						UART_UPPER2->CTRL1 &= ~(1<<9);
					}
					break;
				case RS485_CMD_WRITE_REGS:
					if((s->ptr_no >= 7)&&(s->ptr_no == (s->u16Buffer[6] + 8))) {
...
void Sci2_CommonUpper_Tx_Deal(struct RS485MSG *s) {
	...
	while(!((UART_UPPER2->STS)&(1<<7)));				// 1<<6 也可以
	if( s->ptr_no < s -> AckLenth) {
        UART_UPPER2->DAT = s->u16Buffer[s->ptr_no];	//load data
		s->ptr_no++;
		if((s->ptr_no == 19) ||(s->ptr_no == 39) ||(s->ptr_no == 59)) {
			// delayFlag = 1;
		}
...
void App_CommonUpperSCI2(struct RS485MSG *s)
{
	...
	switch(s->csr) {
		...
		case RS485_STA_TX_COMPLETE: {
			if(gu8_TxFinishFlag_SCI2) {
				...
				s->ptr_no = 0;
				UART_UPPER2->CTRL1 |= (1<<1);	//使能接收
				UART_UPPER2->CTRL1 |= (1<<9);	//使能接收中断
				gu8_TxEnable_SCI2 = 0;
				MCUO_RS485_EN2 = 1;
			}
			break;
	...
	// Sci2_CommonUpper_FaultChk();	//没必要在这加
}
...
void Sci3_CommonUpper_FaultChk(void) {
	...
	if(UART_UPPER3->STS&&(1<<13)) {		//接收溢出错误，RXNEIE或EIE使能产生中断，开
		(void)UART_UPPER3->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER3->DAT;
		FaultCnt++;
	}

	if(UART_UPPER3->STS&&(1<<14)) {		//检测到噪声，默认开，不开的话CR3的ONEBIT置1，不开
		(void)UART_UPPER3->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER3->DAT;
		FaultCnt++;
	}

	if(UART_UPPER3->STS&&(1<<15)) {		//帧错误，USART_CR3的EIE使能中断，开
		(void)UART_UPPER3->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER3->DAT;
		FaultCnt++;
	}

	if(UART_UPPER3->STS&&(1<<12)) {		//奇偶校验错误标志 USART_CTRL1的PEIE使能该中断，不开
		(void)UART_UPPER3->STS;			/* 先读STS，再读DAT */
		(void)UART_UPPER3->DAT;
		FaultCnt++;
	}
...
void Sci3_CommonUpper_Rx_Deal(struct RS485MSG *s) {
	...
	UART_UPPER3->CTRL1 &= ~(1<<9);		//和上面那句话二选一
	s->u16Buffer[s->ptr_no] = UART_UPPER3->DAT; 				// 从RXFIFO 中读取接收到的数据
	if((s->ptr_no == 0)&&(s->u16Buffer[0] != RS485_SLAVE_ADDR)&&(s->u16Buffer[0] != RS485_BROADCAST_ADDR)) {
		s->ptr_no = 0;
		s->u16Buffer[0] = 0;
	}
			...
			switch(s->enRs485CmdType) {
				...
				case RS485_CMD_WRITE_REG:
					if (s->ptr_no == 7) {	//	receive complete
						...
						//RCSTA1bits.CREN = 0;  //禁止接收
						//RC1IE = 0;			// 禁止EUSART2 接收中断
						UART_UPPER3->CTRL1 &= ~(1<<1);
						UART_UPPER3->CTRL1 &= ~(1<<9);
					}
					break;
				case RS485_CMD_WRITE_REGS:
					if((s->ptr_no >= 7)&&(s->ptr_no == (s->u16Buffer[6] + 8))) {
...
void Sci3_CommonUpper_Tx_Deal(struct RS485MSG *s) {
	...
	#if 0
	if(gu16_CommuErrCnt_SCI3) {	//出现错误也得把数据全部接收完，然后不回复
		s->ptr_no = 0;
		s->csr = RS485_STA_TX_COMPLETE;
	...
	while(!((UART_UPPER3->STS)&(1<<7)));				// 1<<6 也可以
	if( s->ptr_no < s -> AckLenth) {
        UART_UPPER3->DAT = s->u16Buffer[s->ptr_no];	//load data
		s->ptr_no++;
		if((s->ptr_no == 19) ||(s->ptr_no == 39) ||(s->ptr_no == 59)) {
			delayFlag = 1;
		}
	...
	switch(s->csr) {
		...
		case RS485_STA_TX_COMPLETE: {
			if(gu8_TxFinishFlag_SCI3) {
				...
				s->ptr_no = 0;
				UART_UPPER3->CTRL1 |= (1<<1);	//使能接收
				UART_UPPER3->CTRL1 |= (1<<9);	//使能接收中断
				gu8_TxEnable_SCI3 = 0;
				MCUO_RS485_EN3 = 1;
			}
			break;
...
void Sci_WrRegs_0x10_FlashConnect(struct RS485MSG *s) {
	...
	if(u16WrRegNum == 1) {
		#if 0
		if(TRUE != FlashWrite_DoubleWord(FLASH_ADDR_UPDATE_FLAG, FLASH_TO_IAP_VALUE, FLASH_TO_IAP_VALUE)) {
		...
		#endif
		BkpRam_WriteUINT16(BKP_SRAM_ADDR_UPDATE_FLAG, BKP_SRAM_TO_IAP_VALUE);
		u8FlashUpdateE2PROM = 1;
	}
	else {
		s ->AckType = RS485_ACK_NEG;
