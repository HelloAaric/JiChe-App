...
UINT8 BMS_LOG_RECORD[EVENT_RECORD_LENGTH][16] = {0};		//0是事件编号，1是与上一个事件的时间间隔
LOG_RECORD_FLAG LogRecord_Flag;
UINT8 gu8_Reset_EventRecord = 0;


UINT8 LogEvent_TimeMap(UINT32* Time_S_Cnt)
{
	UINT8 result = 0;
...
void LogEvent_FLASH(LogEventArray event, UINT32* Time_S_Cnt)
{
	UINT16 temp = 0;
	...
	BMS_LOG_RECORD[BMS_LOG_POINT][1] = LogEvent_TimeMap(Time_S_Cnt);
	BMS_LOG_RECORD[BMS_LOG_POINT][2] = 0;
	BMS_LOG_RECORD[BMS_LOG_POINT][3] = 0;
	...
	// TODO，测试一下这里有无问题
	runtimeLOG_Write_LogWord((UINT32*)&BMS_LOG_RECORD[BMS_LOG_POINT-1][0]);
}
...
void LogEvent_Record(UINT8 temp, LogEventArray event, UINT32* Time_S_Cnt)
{
	...
	if(BMS_START_UP == event) {
		if(LogRecord_Flag.bits.Log_StartUp) {	//直接使用，直接修改，别的不能自己修改
			LogEvent_FLASH(event, Time_S_Cnt);
...
void Log_FromFlash(void)
{
	gu8_Reset_EventRecord = runtimeLOG_Get_LogPage(BMS_LOG_RECORD);
}

void Log_OnGoing(void)
{
	UINT8 temp = 0;

	static UINT8 su8_LogStatus = 0;
