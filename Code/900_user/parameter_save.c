...
// 1，读取完毕
UINT8 ReadEEPROM_ByteData_StartUp_Part(void)
{
	UINT8 result = 0;
	...
	static UINT8 su8_GetData_Part = 0;
...
void EEPROM_ResetData_OtherToDefault(void) {
	...
	SwitchResetData_EEPROM();							//Switch功能存储
	...
	switch(su8_StartUp_Flag) {
		case 0:
			...
			if(EEPROM_VALUE_BEGIN_FLAG == ReadEEPROM_Word_NoZone(EEPROM_ADDR_PASS)) {
				// ReadEEPROM_ByteData_StartUp();
				if(ReadEEPROM_ByteData_StartUp_Part()) {
					su8_StartUp_Flag = 1;
				}
			}
			break;
